            ...
            <div class="bg-[#79b5ff] dark:bg-[#263238] overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-[#333333] dark:text-[#E0E0E0]">
                    <form action="{{ route('grades.update', $grade->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="user_id" name="user_id" class="bg-[#C8E6C9] dark:bg-[#2E3B4E] mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $user->id == $grade->user_id ? 'selected' : '' }}>{{ $user->first_name }} {{ $user->last_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            <label for="assignment_name" class="block text-sm font-medium text-[#333333] dark:text-[#E0E0E0]">{{ __('Opdracht Naam') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="grade" class="block text-sm font-medium text-[#333333] dark:text-[#E0E0E0]">{{ __('Cijfer') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="description" class="block text-sm font-medium text-[#333333] dark:text-[#E0E0E0]">{{ __('Beschrijving') }}</label>
