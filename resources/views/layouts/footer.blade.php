<footer class="bg-[#4A90E2] dark:bg-[#3B5998]">
  <div class="w-full max-w-screen-xl mx-auto p-4 md:py-8">
      <div class="sm:flex sm:items-center sm:justify-between">

          <a href="/" class="flex items-center mb-4 sm:mb-0 space-x-3 rtl:space-x-reverse">
              <span class="self-center text-2xl font-semibold whitespace-nowrap text-[#333333] dark:text-[#E0E0E0]">EduTrack</span>
          </a>

          <ul class="flex flex-wrap items-center mb-6 text-sm font-medium text-[#333333] dark:text-[#E0E0E0] sm:mb-0">
              <li>
                  <a href="#" class="hover:text-[#50E3C2] hover:dark:text-[#FF6F61] hover:underline me-4 md:me-6"></a>
              </li>
              <li>
                  <a href="#" class="hover:text-[#50E3C2] hover:dark:text-[#FF6F61] hover:underline me-4 md:me-6"></a>
              </li>
              <li>
                  <a href="#" class="hover:text-[#50E3C2] hover:dark:text-[#FF6F61] hover:underline me-4 md:me-6"></a>
              </li>
              <li>
                  <a href="#" class="hover:text-[#50E3C2] hover:dark:text-[#FF6F61] hover:underline"></a>
              </li>
          </ul>
          
      </div>

      <hr class="my-6 border-gray-700 sm:mx-auto dark:border-gray-900 lg:my-8" />
      <span class="block text-sm text-[#333333] dark:text-[#E0E0E0] sm:text-center">© 2024 <a href="/" class="hover:underline">EduTrack</a>. All Rights Reserved.</span>
  </div>
</footer>

